...
def test_case_checker(stu_func, task_id=0):
    ...
    rtol = 1e-05
    if ('meanfield' in this_dir.lower()) and ('3' in str(task_id)):
        atol = 1./256.
    else:
        atol = 1e-08
    ...
    for (test_args, test_kwargs, correct_sol) in test_case_loader(test_db_npz):
        ...
        if isinstance(correct_sol, np.ndarray):
            equality_array = np.isclose(stu_sol, correct_sol, rtol=rtol, atol=atol, equal_nan=True)
...
def show_test_cases(test_func, task_id=0):
    ...
    for i in range(ref_images.shape[0]):
        ...
        if np.allclose(ref_image, test_image, rtol=rtol, atol=atol, equal_nan=True):
            if visualize_:
            ...
            ineq_idxs = np.array(np.where(np.logical_not(np.isclose(ref_image, test_image, rtol=rtol, atol=atol, equal_nan=True))))[:,0].tolist()
